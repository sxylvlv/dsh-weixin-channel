...
- **主动发视频已接入**（2026-09-14 真机验证：3 秒 mp4 正常送达并播放）。配方要点：引用参数取自上传响应头 `x-encrypted-param`、`aes_key = base64(hex)`、带 `encrypt_type: 1`、**不能带 `full_url`**。
